# Hi there, I'm Alexandre! 👋

### Cybersecurity Student | Blue Team & Threat Intelligence

I am a cybersecurity student at **École 2600**, specializing in **Threat Intelligence**, **SOC/DFIR**, and **OSINT**. Currently working in a CERT at the French Ministry of the Armed Forces. My goal is to build tools that turn raw data into actionable intelligence.

Based in Paris 🇫🇷, open to international opportunities 🌍.

---

### 🛠️ Technical Stack

* **Detection & Response:** ELK/Kibana, Splunk, Sigma rules, KQL, YARA, TheHive/Cortex, Shuffle (SOAR).
* **CTI & OSINT:** OpenCTI, n8n, Python (Scraping, Automation), Graph Analysis.
* **Forensics:** Volatility, Autopsy, Zimmerman toolkit, MITRE ATT&CK mapping.
* **Infra & DevOps:** Docker, AWS (Terraform/HCL), Packer, Ansible, Linux.

---

### 🔭 Featured Projects

* ⚡ **[CyberZap / LobsterIntel](https://github.com/Iskandeur/cyber-zap-public):** Automated CTI pipeline — collects, scores and distributes threat intelligence via n8n (50+ nodes), evolving towards an agentic AI architecture (OpenClaw).
* 🕵️ **[WhatsApp Social Graph](https://github.com/Iskandeur/whatsapp-social-graph):** Link analysis tool generating dynamic social graphs from messaging metadata exports.
* ☁️ **[Auto 3-Tier AWS](https://github.com/Iskandeur/auto-3-tier-aws):** Automated deployment of a secure, highly available AWS infrastructure using Immutable Infrastructure principles.
* 🤖 **[Adversarial LLM PoC](https://github.com/Iskandeur/adversarial-llm-poc):** Research on LLM vulnerabilities and security posture improvement.

---

### ⚡ Fun Fact & Philosophy

* 🗣️ I speak French, English, Russian, Spanish, Italian... and **Toki Pona**! *pona tawa sina!*
* 💡 Teaching and mentoring in engineering / electronics during my spare time.
* 🧠 Strong interest in metacognition, philosophy and consciousness theories.

[🌐 Website](https://alexandre-pinoteau.fr/) • [👔 LinkedIn](https://www.linkedin.com/in/p1n0t34u/) • [📧 Email](mailto:alexandre.pinoteau@protonmail.com)
